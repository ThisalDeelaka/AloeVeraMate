"""
Export Stage A model for production serving
"""
import json
...
def export_stage_a(
    ...
    """Export Stage A model with metadata"""
    ...
    # Create metadata
    metadata = {
        "model_type": "stage_a_binary",
        "architecture": "efficientnet_v2_s",
        "stage": "A",
        "description": "Binary classifier: Healthy vs Unhealthy",
        "num_classes": 2,
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("Stage A Export Summary")
    print("=" * 60)
    print(f"Model Type: {metadata['model_type']}")
    print(f"Architecture: {metadata['architecture']}")
    print(f"Classes: {metadata['num_classes']} ({', '.join(STAGE_A_CLASS_NAMES)})")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Export Stage A model")
